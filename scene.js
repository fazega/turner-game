...
renderer.domElement.addEventListener('pointerdown',e=>{if(quest.isOpen())return;drag=true;pointerStart={x:e.clientX,y:e.clientY,button:e.button};dragDistance=0;renderer.domElement.setPointerCapture(e.pointerId)});
...
addEventListener('keydown',e=>{
 ...
 if(['Space','ArrowUp','ArrowDown'].includes(e.code))e.preventDefault();keys.add(e.code);keys.add(e.key.toLowerCase());
...
addEventListener('keyup',e=>{keys.delete(e.code);keys.delete(e.key.toLowerCase())});addEventListener('blur',()=>{keys.clear();drag=false});
